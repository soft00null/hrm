...
async function getOrganizationFromContext(req) {
  ...
  console.log(`[INFO] Using default organization ID: Saijyot`);
  return "Saijyot";
}
...
const openAiTools = [
  {
    ...
    description:
      "Handles new/reschedule/cancel appointment. 'new' => send interactive form => user fills => create doc in /Organisation/Saijyot/Appointment",
    parameters: {
      type: "object",
      properties: {
        action: { type: "string" },
  ...
  },
  {
    name: "support_flow",
    description:
      "Send 'support' template => user fills => parse => create doc in /Organisation/Saijyot/Ticket (support ticket).",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: `
Answers user questions about Saijyot Multispeciality Hospital using only the loaded knowledgeBase text. 
...
async function getSaijyotOrganisationRef() {
  return getOrganisationRef("Saijyot");
}
...
// ----------------------------------------------------------------------
async function sendWelcomeTemplate(userPhone, pocName = "User", orgName = "Saijyot Hospital") {
 ...
 */
async function processWelcomeFormSubmission(pocRef, flowData, orgId = "Saijyot") {
 ...
 */
async function getOrCreatePoCByPhone(phone, contactName, orgId = "Saijyot") {
...
async function sendAppointmentFlow(userPhone) {
  ...
  let payload = {
    ...
    interactive: {
      type: "flow",
      header: { type: "text", text: "Saijyot Hospital" },
...
async function createAppointmentDoc(pocRef, userPhone, flowData, orgId = "Saijyot") {
...
async function createSupportTicket(pocRef, userId, flowData, orgId = "Saijyot") {
...
// Checkin flow
// ----------------------------------------------------------------------
async function createCheckinDoc(pocRef, userPhone, orgId = "Saijyot") {
...
async function checkKnowledgeFullWithGpt(userQuery, fullKnowledgeText) {
  try {
    const content = `
You are Saijyot Hospital's helpful assistant. 
...
async function knowledgeLookupImpl(userQuery) {
  ...
  return "No direct info found in the knowledge base. Please ask more about Saijyot hospital!";
}
...
function smallTalkImpl(userMessage) {
  ...
  return `Hello! Saijyot Hospital is here to help. How can we assist you today?`;
}

// ----------------------------------------------------------------------
// GPT-based Symptom Classification + Triage
// ----------------------------------------------------------------------
async function symptomAssessmentImpl(userSymptom, orgId = "Saijyot") {
...
// ----------------------------------------------------------------------
async function findDoctorsByFuzzySpecialty(specialty, orgId = "Saijyot") {
  try {
...
// ----------------------------------------------------------------------
async function updateAppointmentFeedback(flowData, orgId = "Saijyot") {
...
// ----------------------------------------------------------------------
async function handleLocalFunctionCall(name, args, fromPhone, userId, userQuery, orgId = "Saijyot") {
  ...
  switch (name) {
    ...
    default:
      return "Kindly only ask anything related to Saijyot Hospital.";
  }
}

async function handleOpenAiFunctionCall(fCall, fromPhone, userId, userQuery, orgId = "Saijyot") {
...
const systemMessage = {
  role: "system",
  content: `
You are Saijyot Hospital's Chatbot in English. 
...
app.get("/migrate-pocs", async (req, res) => {
  try {
    ...
    return res.status(200).json({ 
      success: true, 
      message: `Updated ${count1} PoCs with addedBy="Saijyot", ${count2} with lastSeen, and ${count3} with registered=true`,
...
app.get("/", (req, res) => {
  res.status(200).json({
    message: "Saijyot Hospital HRM Bot is running!",
    timestamp: formatISTTimestamp(),
